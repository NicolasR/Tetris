...
\textit{getAllPos} = $\left\{~(x,y) \in Pair<1..\textit{getSize}(B),1..\textit{getSize}(B)>~|~\textit{hasPos(B,x,y)}~\right\}$ \\
\textit{getAllPos} = $\left\{~(x,y) \in \textit{\textit{getAllPos}(B)}~|~\neg \exists (x,y2) \in \textit{\textit{getAllPos}}(B), y2>y~\right\}$ \\
>>>>>>> 921652edb37e11eaecc3a5d430cb491251671e2e:rapport.tex
...
\underline{précondition}: \=\textit{clean}(B) \underline{require} \textit{isBottom}(B)\\\\
...
\textit{getTetris}(\textit{goRight}(J)) = \textbf{Tetris:}\textit{goRight}(\textit{getTetris}(J)e)\\
