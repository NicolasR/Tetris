...
\textit{isBlock}(B) $<=>$ \textit{getYMinBlock}(B) $>=$ 0 \&\& \textit{getYMinBlock}(B) $<=$ \textit{getgrid}(\textit{getHeight}(B)))\\
...
\underline{précondition}: \=\textit{startGame}(J) \underline{require} \textit{canPlay}(J)\\
...
$\textit{getTetris}(J) \neq null$\\
 ...
 \item[$\circ$] \textit{init}\\
\textit{getTetris}(J) = \textbf{Tetris:}\textit{init}(10,22)\\
...
\item[$\circ$] goDown\\
\textbf{Board:}\textit{isBottom}(\textit{getBoard}(T)) $=>$ \textit{getScore}(\textit{goDown}(T)) = \textit{getScore}(T) + 20 + \textbf{Board:}\textit{getNbLastCleaned}(\textit{getBoard}(\textit{goDown}(T)))*50\\
$\neg$\textbf{Board:}\textit{isBottom}(\textit{getBoard}(T)) $=>$ \textit{getScore}(\textit{goDown}(T)) = \textit{getScore}(T)\\
...
\item[$\circ$] step\\
\textbf{Board:}\textit{isBottom}(\textit{getBoard}(T)) \&\& \textit{needNext}(B) $=>$\\ \textit{getScore}(\textit{step}(T)) = \textit{getScore}(T)\\
\textbf{Board:}\textit{isBottom}(\textit{getBoard}(T)) \&\& $\neg$\textit{needNext}(B) $=>$\\ \textit{getScore}(\textit{step}(T)) = \textit{getScore}(T) + 20 + \textbf{Board:}\textit{getNbLastCleaned}(\textit{getBoard}(\textit{step}(T)))*50\\
...
$\neg$\textbf{Board:}\textit{isBottom}(\textit{getBoard}(T)) $=>$ \textit{needNext}(\textit{step}(T)) = \textit{needNext}(T)\\
